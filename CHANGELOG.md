...
- Soaring Winds can now be applied to modded gliding chestplates. It draws from the `betterarcheology:soaring_winds_items` item tag instead of `c:elytras` directly, so packs can widen it — e.g. to an Apotheosis chestplate with the Winged affix. On Fabric it also recognises chestplates that provide gliding themselves, instead of only accepting a literal elytra (#4)
